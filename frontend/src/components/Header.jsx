...
const Header = () => {
  ...
  return (
    ...
    >
      {/* Top bar with achievements - hidden on mobile */}
      <div className={`hidden sm:block border-b border-white/5 transition-all duration-300 ${scrolled ? 'py-1' : 'py-2'}`}>
          ...
          <div className="flex items-center gap-3 sm:gap-6 overflow-x-auto hide-scrollbar">
            {stats.map((stat, index) => (
              <div key={index} className="flex items-center gap-1 sm:gap-2 whitespace-nowrap">
                ...
                <span className="text-gray-400 text-[10px] sm:text-xs font-medium">{stat.label}</span>
              </div>
            ))}
          </div>
